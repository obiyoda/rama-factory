  ...
  (let [root (temp-dir)]
    (try
      (let [result (generator/create-app! "Invoice App" (path root "invoice-app") {})]
        ...
        (is (.exists (io/file (:root result) "package.json")))
        (is (.exists (io/file (:root result) "pnpm-lock.yaml")))
        (is (.exists (io/file (:root result) "vite.config.mjs")))
        (is (.exists (io/file (:root result) "assets/app.css")))
        (is (.exists (io/file (:root result) "assets/app.js")))
        ...
        (is (str/includes? (slurp (io/file (:root result) "deps.edn"))
                           "com.rpl/rama"))
        (is (str/includes? (slurp (io/file (:root result) "package.json"))
                           "basecoat-css"))
        (is (str/includes? (slurp (io/file (:root result) "package.json"))
                           "\"packageManager\": \"pnpm@10.8.1\""))
        (is (str/includes? (slurp (io/file (:root result) "pnpm-lock.yaml"))
                           "lockfileVersion"))
        (is (str/includes? (slurp (io/file (:root result) "devenv.nix"))
                           "processes.assets"))
        (is (str/includes? (slurp (io/file (:root result) "devenv.nix"))
                           "pkgs.pnpm"))
        (is (str/includes? (slurp (io/file (:root result) "devenv.nix"))
                           "NODE_OPTIONS=--no-warnings pnpm install --frozen-lockfile")))
      (finally
